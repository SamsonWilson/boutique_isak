...
class InventaireCreateView(View):
    ...
    def get(self, request, boutique_id):
        ...
        if date_debut_str and date_fin_str:
            try:
                # 🔹 créer datetime naive depuis string
                debut_naive = datetime.strptime(date_debut_str, "%Y-%m-%d")
                fin_naive = datetime.strptime(date_fin_str, "%Y-%m-%d")

                # 🔹 inclure la journée entière → on ajoute 23h59m59s
                fin_naive = fin_naive + timedelta(days=1) - timedelta(seconds=1)

                # 🔹 rendre aware selon le fuseau actuel de Django
                ...
                # 🔹 filtrer avec des datetime aware
                mouvements = mouvements.filter(date_mouvement__range=(debut, fin))
