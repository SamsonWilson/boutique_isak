...
class ProduitCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        C'est ici qu'on assigne l'utilisateur.
        """
        # 1. On prépare l'objet à partir du formulaire, mais sans le sauvegarder en BDD
        #    On l'assigne à self.object pour que le reste de la logique de la CBV fonctionne
        self.object = form.save(commit=False)
        
        # 2. On assigne l'utilisateur connecté (accessible via self.request.user)
        self.object.utilisateur = self.request.user
        
        # 3. On sauvegarde l'objet en base de données.
        self.object.save()
        
        # 4. On laisse la méthode parente (de CreateView) gérer la redirection.
