...
class StockUpdateView(LoginRequiredMixin,UpdateView):
    ...
    def form_valid(self, form):
        mouvement = self.object
        ancien_delta = mouvement.quantite if mouvement.type_mouvement == 'entrée' else -mouvement.quantite
        new_delta = form.cleaned_data['quantite'] if form.cleaned_data['type_mouvement'] == 'entrée' else -form.cleaned_data['quantite']

        stock_courant = StockCourant.objects.get(boutique=mouvement.boutique, produit=mouvement.produit)
        # Ajuster : enlever l’ancien mouvement puis ajouter le nouveau
        stock_courant.quantite -= ancien_delta
        stock_courant.quantite += new_delta
        if stock_courant.quantite < 0:
            form.add_error('quantite', "Stock insuffisant après modification.")
            return self.form_invalid(form)
        stock_courant.save()
