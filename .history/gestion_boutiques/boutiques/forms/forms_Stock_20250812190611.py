...
class StockUpdateForm(forms.ModelForm):
    quantite_a_ajouter = forms.IntegerField(
        ...
        required=False,
        initial=0,
        help_text="Nombre positif = ajout, nombre négatif = retrait",
        widget=forms.NumberInput(
            attrs={
                'class': 'form-control',
                'id': 'id_quantite_a_ajouter'
            }
        )
    )
    ...
    class Meta:
        model = Stock
        fields = ['produit', 'quantite_disponible', 'quantite_a_ajouter']
        widgets = {
            'produit': forms.TextInput(attrs={'class': 'form-control', 'readonly': 'readonly'}),
            'quantite_disponible': forms.NumberInput(attrs={
                'class': 'form-control',
                'readonly': 'readonly',
                'id': 'id_quantite_disponible'
            }),
        }