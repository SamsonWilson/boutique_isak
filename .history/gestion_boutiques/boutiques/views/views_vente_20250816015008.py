...
from .models import Client, Vente, DetailVente, Produit
...
class VenteCreateView(LoginRequiredMixin, View):
    ...
    success_url = "/ventes/"  # tu peux rediriger vers une autre URL

    def form_valid(self, form):
            items_json = self.request.POST.get("items")
            if not items_json:
                form.add_error(None, "Aucun produit sélectionné.")
            ...
            # 2. Calcul du total
            total = sum(Decimal(i["prix"]) * int(i["quantite"]) for i in items)
...
class ProductSearchView(View):
    def get(self, request, *args, **kwargs):
        q = request.GET.get("q", "")
        produits = Produit.objects.filter(nom__icontains=q)[:10]
        data = [{"id": p.id, "nom": p.nom, "prix": float(p.prix)} for p in produits]
