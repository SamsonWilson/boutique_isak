...
class VenteCreateView(FormView):
    ...
    def form_valid(self, form):
        ...
        if not self.request.user.boutique:
            form.add_error(None, "⚠️ Utilisateur non assigné à une boutique.")
            return self.form_invalid(form)
...
class VenteListView(ListView):
    model = Vente
    template_name = "ventes/vente_liste.html"
    ...
    def get_queryset(self):
        ...
        if date_debut:
            qs = qs.filter(date__date__gte=parse_date(date_debut))
        if date_fin:
            qs = qs.filter(date__date__lte=parse_date(date_fin))
