...
class RoleMiddleware:
    """
    Middleware qui contrôle l'accès en fonction du rôle utilisateur
    """
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        if not request.user.is_authenticated:
            return None  # géré par les vues de login

        # URLs à exclure des restrictions (accueil, login, logout)
        safe_urls = [
            reverse("accueil"),
            reverse("login"),
            reverse("logout"),
        ]
        if request.path in safe_urls:
            return None  # ne pas bloquer cette vue

        # --- Règles d'accès ---
        role_permissions = {
            "admin":    ["/utilisateurs/", "/produits/", "/stocks/", "/ventes/", "/boutiques/"],
            "gerant":   ["/produits/", "/stocks/", "/ventes/"],
            "vendeur":  ["/ventes/"],
        }
        ...
        # Vérifie que l’URL demandée correspond à un chemin autorisé
