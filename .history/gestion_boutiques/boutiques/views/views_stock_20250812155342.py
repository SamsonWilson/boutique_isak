...
class StockUpdateView(UpdateView):
    model = Stock
    form_class = StockForm
    template_name = 's/stock_update_form.html'
    success_url = reverse_lazy('stock_list')
    def form_valid(self, form):
        form.instance.utilisateur = self.request.user

        try:
            # Chercher le stock existant pour cette boutique et produit
            existing_stock = Stock.objects.get(
                boutique=form.cleaned_data['boutique'],
                produit=form.cleaned_data['produit']
            )

            if form.cleaned_data['type_mouvement'] == 'entrée':
                # Ajouter quantité
                existing_stock.quantite = F('quantite') + form.cleaned_data['quantite']
            else:  # sortie
                existing_stock.quantite = F('quantite') - form.cleaned_data['quantite']

            existing_stock.save()
            return super().form_valid(form)  # Pour redirection ou autre

        except Stock.DoesNotExist:
            # Si pas encore existant → créer normalement
            if form.cleaned_data['type_mouvement'] == 'sortie':
                # Empêcher stock négatif
                form.instance.quantite = 0 - form.cleaned_data['quantite']
            return super().form_valid(form)
