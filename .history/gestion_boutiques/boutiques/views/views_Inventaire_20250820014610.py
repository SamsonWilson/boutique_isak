...
class InventaireCreateView(View):
    ...
    def post(self, request, boutique_id):
        ...
        # 2️⃣ Parcourir les stocks de la boutique
        stocks = StockCourant.objects.filter(boutique=boutique).select_related("produit")
        for stock in stocks:
            ...
            # ⚡ Enregistrer le détail de l'inventaire
            ...
            # ⚡ Créer un mouvement stock si écart
            if ecart != 0:
                MouvementStock.objects.create(
                    ...
                    description=f"Ajustement après inventaire #{inventaire.id}",
                    utilisateur=request.user
                )
